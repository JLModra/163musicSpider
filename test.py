# -*- coding: utf-8 -*-
"""
Created on Sun Apr 24 13:17:14 2016
...
#获取歌单的信息
def get_music_list_info(url):
    music_list_info.remove()
    data = {'id' : str(url[33:])}
    music_list_page = requests.get(url, headers=header, data=json.dumps(data))
    soup = BeautifulSoup(music_list_page.text, "html5lib")
    try:
        if (soup.find_all(class_='note s-fc3')[0].string == '很抱歉，你要查找的网页找不到'):
            print url+'这个url是无效的'
            return 
        else:
...
pool = Pool()
#print url
pool.map(get_music_list_info,url)
pool.close()
pool.join()
